/**
 * Twitch channel plugin for Verso.
 *
...
import { buildChannelConfigSchema } from "verso/plugin-sdk";
 ...
 * for Verso. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: VersoConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: VersoConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: VersoConfig): boolean => {
