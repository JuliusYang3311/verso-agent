...
Verso extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
verso message send --channel zalouser --target <threadId> --message "Hello from Verso!"
```
...
```bash
verso channels login --channel zalouser              # Login via QR
verso channels login --channel zalouser --account work
verso channels status --probe
verso channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
verso directory self --channel zalouser
verso directory peers list --channel zalouser --query "name"
verso directory groups list --channel zalouser --query "work"
verso directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
verso message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
verso message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `verso channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
verso channels login --channel zalouser --account work
verso message send --channel zalouser --account work --target <id> --message "Hello"
