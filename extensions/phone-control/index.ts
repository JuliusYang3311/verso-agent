import type { VersoPluginApi, VersoPluginService } from "verso/plugin-sdk";
...
function normalizeDenyList(cfg: VersoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: VersoPluginApi["config"]): string[] {
...
export default function register(api: VersoPluginApi) {
  ...
  const timerService: VersoPluginService = {
