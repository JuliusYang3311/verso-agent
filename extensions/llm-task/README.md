...
`verso.invoke --each`) without adding new Verso code per workflow.
...
This extension depends on Verso internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Verso extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.verso/extensions` as a standalone plugin directory.
