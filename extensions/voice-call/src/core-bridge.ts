...
function resolveVersoRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.VERSO_ROOT?.trim() || process.env.VERSO_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set VERSO_ROOT (or legacy VERSO_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveVersoRoot();
