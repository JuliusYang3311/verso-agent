...
import { approveDevicePairing, listDevicePairing } from "verso/plugin-sdk";
...
function resolveGatewayPort(cfg: VersoPluginApi["config"]): number {
  const envRaw =
...
async function resolveTailnetHost(api: VersoPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: VersoPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: VersoPluginApi): Promise<ResolveUrlResult> {
