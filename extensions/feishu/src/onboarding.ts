...
import { addWildcardAllowFrom, DEFAULT_ACCOUNT_ID, formatDocsLink } from "verso/plugin-sdk";
...
function setFeishuDmPolicy(cfg: VersoConfig, dmPolicy: DmPolicy): VersoConfig {
  const allowFrom =
    dmPolicy === "open"
...
function setFeishuAllowFrom(cfg: VersoConfig, allowFrom: string[]): VersoConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
function setFeishuGroupAllowFrom(cfg: VersoConfig, groupAllowFrom: string[]): VersoConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
