...
const CLIENT_ID_KEYS = ["VERSO_GEMINI_OAUTH_CLIENT_ID", "GEMINI_CLI_OAUTH_CLIENT_ID"];
const CLIENT_SECRET_KEYS = ["VERSO_GEMINI_OAUTH_CLIENT_SECRET", "GEMINI_CLI_OAUTH_CLIENT_SECRET"];
...
async function discoverProject(accessToken: string, ctx?: GeminiCliOAuthContext): Promise<string> {
  ...
  const headers = {
    ...
    "X-Goog-Api-Client": "gl-node/verso",
  };
