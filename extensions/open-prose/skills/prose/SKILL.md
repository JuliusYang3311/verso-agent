...
metadata: { "verso": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Verso `sessions_spawn`
- **File I/O** == Verso `read`/`write`
- **Remote fetch** == Verso `web_fetch` (or `exec` with curl when POST is required)
