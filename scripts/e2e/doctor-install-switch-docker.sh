...
IMAGE_NAME="verso-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/verso-bin:$PATH"
  mkdir -p /tmp/verso-bin

  cat > /tmp/verso-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/verso-bin/systemctl

  cat > /tmp/verso-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/verso-bin/loginctl
...
<<<<<<< HEAD
  npm_bin="/tmp/npm-prefix/bin/verso"
  npm_entry="/tmp/npm-prefix/lib/node_modules/verso/verso.mjs"
  git_entry="/app/verso.mjs"
=======
...
>>>>>>> upstream/main
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/verso-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/verso-gateway.service"
    if [ ! -f "$unit_path" ]; then
