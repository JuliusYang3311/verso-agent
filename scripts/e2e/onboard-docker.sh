...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/verso-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/verso-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/verso-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.verso/verso.json"
    sessions_dir="$HOME/.verso/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.verso/verso.json" <<'"'"'JSON'"'"'
{
