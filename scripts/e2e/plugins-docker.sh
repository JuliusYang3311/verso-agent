...
echo "Running plugins Docker E2E..."
  ...
  home_dir=$(mktemp -d "/tmp/verso-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.verso/extensions"

  cat > "$HOME/.verso/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/verso-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@verso/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/verso-plugin-dir.XXXXXX")"
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/verso-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@verso/demo-plugin-file",
  "version": "0.0.1",
  "verso": { "extensions": ["./index.js"] }
}
JSON
