...
const shouldBuild = () => {
  if (env.VERSO_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.VERSO_RUNNER_LOG === "0") return;
  process.stderr.write(`[verso] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["verso.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
