...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/verso/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Verso Re-Auth" -c "After getting key, run: ssh $SERVER '~/verso/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
