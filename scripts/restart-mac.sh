#!/usr/bin/env bash
# Reset Verso like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CLAWDBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Verso.app/Contents/MacOS/Verso"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Verso"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Verso"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Verso"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.molt.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/verso-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLAWDBOT_RESTART_LOG:-/tmp/verso-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CLAWDBOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.verso/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node verso.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.verso/disable-launchagent"
      log ""
...
kill_all_verso() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "Verso" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
log "==> Killing existing Verso instances"
kill_all_verso
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Verso"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Verso.app" ]]; then
    APP_BUNDLE="/Applications/Verso.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Verso.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Verso.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Verso.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CLAWDBOT_APP_BUNDLE to your installed Verso.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node verso.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node verso.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".verso", "verso.json"), "utf8");
