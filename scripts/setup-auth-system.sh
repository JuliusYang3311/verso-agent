#!/bin/bash
# Setup Verso Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'verso-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Verso message: Send warning via Verso itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/verso-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/verso-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/verso-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now verso-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status verso-auth-monitor.timer"
echo ""
echo "Quick commands:"
