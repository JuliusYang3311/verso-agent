...
#
# Suggested cron: */30 * * * * /home/admin/verso/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Verso notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., verso-alerts)
...
STATE_FILE="$HOME/.verso/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Verso if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use verso
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Verso to $NOTIFY_PHONE..."
            verso send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Verso is down. Run: ssh l36 '~/verso/scripts/mobile-reauth.sh'" "urgent"
    exit 1
