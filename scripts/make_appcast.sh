...
ZIP=${1:?"Usage: $0 Verso-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/verso/verso/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Verso-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/verso/verso/releases/download/v${VERSION}/"}
