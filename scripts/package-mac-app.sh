...
# Build and bundle Verso into a minimal .app we can open.
# Outputs to dist/Verso.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Verso.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/verso/verso/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Verso/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :VersoBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :VersoGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Verso"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Verso"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Verso"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Verso" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Verso/Resources/Verso.icns" "$APP_ROOT/Contents/Resources/Verso.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Verso/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying VersoKit resources"
MOLTBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/VersoKit_VersoKit.bundle"
if [ -d "$MOLTBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/VersoKit_VersoKit.bundle"
  cp -R "$MOLTBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/VersoKit_VersoKit.bundle"
else
  echo "WARN: VersoKit resource bundle not found at $MOLTBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Verso"
killall -q Verso 2>/dev/null || true
