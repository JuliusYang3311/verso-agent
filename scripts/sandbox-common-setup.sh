...
BASE_IMAGE="${BASE_IMAGE:-verso-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-verso-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=verso.sandbox=1)
NOTE
