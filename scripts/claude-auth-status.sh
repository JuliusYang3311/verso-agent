...
# Checks both Claude Code and Verso auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
CLAWDBOT_AUTH="$HOME/.verso/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    verso models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    verso_status=$(check_verso_auth 2>/dev/null || true)

    claude_expires=0
    verso_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        verso_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        verso_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CLAWDBOT_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$verso_status" \
        --arg be "$verso_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            verso: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$verso_status" == EXPIRED* ]] || [[ "$verso_status" == MISSING* ]]; then
        echo "CLAWDBOT_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$verso_status" == EXPIRING* ]]; then
...
echo ""
echo "Verso Auth (~/.verso/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'verso doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active verso >/dev/null 2>&1; then
    echo -e "Verso service: ${GREEN}running${NC}"
else
    echo -e "Verso service: ${RED}NOT running${NC}"
fi
