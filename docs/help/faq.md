...
### How do I install Verso on Linux
...
### How do I install Verso on a VPS
...
### Can I ask Verso to update itself
...
verso update
verso update status
verso update --channel stable|beta|dev
verso update --tag <dist-tag|version>
verso update --no-restart
```

If you must automate from an agent:

```bash
verso update --yes --no-restart
verso gateway restart
```
...
`verso onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run Verso with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `verso models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `verso models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `verso models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro/Max)

Yes — via **setup-token**. Verso no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so Verso can keep replying while a provider is rate‑limited.
...
Verso supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Verso fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `verso.json`.

Steps:
1) Enable the plugin: `verso plugins enable google-gemini-cli-auth`
2) Login: `verso models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. Verso needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. Verso runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
any Mac works. Verso’s iMessage integrations run on macOS (BlueBubbles or `imsg`), while
...
### If I buy a Mac mini to run Verso can I connect it to my MacBook Pro
...
- Use `verso nodes status` / `verso nodes list` to see it.
...
- DM your bot, then run `verso logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different Versos
...
If you run Verso via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non‑login shells.
...
This **does not delete your data** - it only changes the Verso code install. Your state
(`~/.verso`) and workspace (`~/verso`) stay untouched.
...
**Verso-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you’re actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run Verso on a dedicated machine
...
Verso is lightweight. For a basic Gateway + one chat channel:
...
### Can I run Verso in a VM and what are the requirements
...
### What is Verso in one paragraph

Verso is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### Whats the value proposition

Verso is not “just a Claude wrapper.” It’s a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for Verso
...
### Can Verso help with lead gen outreach ads and blogs for a SaaS
...
Verso draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

Verso is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use Verso when you
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.verso/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.verso/verso.json`). Precedence is `<workspace>/skills` > `~/.verso/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.verso/verso.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.verso/skills` → bundled → `skills.load.extraDirs`. `versohub` installs into `./skills` by default, which Verso treats as `<workspace>/skills`.
...
Checklist:
- Confirm cron is enabled (`cron.enabled`) and `VERSO_SKIP_CRON` is not set.
- Check the Gateway is running 24/7 (no sleep/restarts).
- Verify timezone settings for the job (`--tz` vs host timezone).

Debug:
```bash
verso cron run <jobId> --force
verso cron runs --id <jobId> --limit 50
```
...
Use **VersoHub** (CLI) or drop skills into your workspace. The macOS Skills UI isn’t available on Linux.
Browse skills at https://versohub.com.

Install the VersoHub CLI (pick one package manager):
...
### Can Verso run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.verso.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `imsg`, `apple-notes`, `apple-reminders`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**  
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-verso-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**  
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Verso can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3) Override the skill metadata (workspace or `~/.verso/skills`) to allow Linux:
   ```markdown
   ---
   name: imsg
   description: iMessage/SMS CLI for listing chats, history, watch, and sending.
   metadata: {"verso":{"os":["darwin","linux"],"requires":{"bins":["imsg"]}}}
   ---
   ```
4) Start a new session so the skills snapshot refreshes.

For iMessage specifically, you can also point `channels.imessage.cliPath` at an SSH wrapper (Verso only needs stdio). See [iMessage](/channels/imessage).
...
VersoHub installs into `./skills` under your current directory (or falls back to your configured Verso workspace); Verso treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.verso/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [VersoHub](/tools/versohub).
...
Verso memory is just Markdown files in the agent workspace:
- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

Verso also runs a **silent pre-compaction memory flush** to remind the model
...
If you don’t set a provider explicitly, Verso auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, Verso
prefers `local`.
...
### Is all data used with Verso saved locally

No - **Verso’s state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.verso` + your workspace directory).
...
### Where does Verso store its data

Everything lives under `$VERSO_STATE_DIR` (default: `~/.verso`):

| Path | Purpose |
|------|---------|
| `$VERSO_STATE_DIR/verso.json` | Main config (JSON5) |
| `$VERSO_STATE_DIR/credentials/oauth.json` | Legacy OAuth import (copied into auth profiles on first use) |
| `$VERSO_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys) |
| `$VERSO_STATE_DIR/agents/<agentId>/agent/auth.json` | Runtime auth cache (managed automatically) |
| `$VERSO_STATE_DIR/credentials/` | Provider state (e.g. `whatsapp/<accountId>/creds.json`) |
| `$VERSO_STATE_DIR/agents/` | Per‑agent state (agentDir + sessions) |
| `$VERSO_STATE_DIR/agents/<agentId>/sessions/` | Conversation history & state (per agent) |
| `$VERSO_STATE_DIR/agents/<agentId>/sessions/sessions.json` | Session metadata (per agent) |

Legacy single‑agent path: `~/.verso/agent/*` (migrated by `verso doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/verso`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.verso`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.verso`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.verso/skills`).

Default workspace is `~/verso`, configurable via:
...
Do **not** commit anything under `~/.verso` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall Verso
...
`workspace` to the repo root. The Verso repo is just source code; keep the
...
Verso reads an optional **JSON5** config from `$VERSO_CONFIG_PATH` (default: `~/.verso/verso.json`):

```
$VERSO_CONFIG_PATH
```

If the file is missing, it uses safe‑ish defaults (including a default workspace of `~/verso`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `VERSO_GATEWAY_TOKEN`).
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: `verso doctor --generate-gateway-token`.
...
key. **Recommended:** run `verso configure --section web` to store it in
...
- Daemons read env vars from `~/.verso/.env` (or the service environment).
...
### Can the Verso browser run headless
...
Check the basics:
- Gateway is running: `verso gateway status`
- Gateway health: `verso status`
- Channel health: `verso channels status`
...
### Can two Versos talk to each other local VPS
...
`verso agent --message ... --deliver`, targeting a chat where the other bot
listens. If one bot is on a remote VPS, point your CLI at that remote Gateway
via SSH/Tailscale (see [Remote access](/gateway/remote)).

Example pattern (run from a machine that can reach the target Gateway):
```bash
verso agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
Recover:
- Restore from backup (git or a copied `~/.verso/verso.json`).
- If you have no backup, re-run `verso doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:
- Use `verso config set` for small changes.
- Use `verso configure` for interactive edits.
...
### How does Verso load environment variables

Verso reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:
...
1) Put the missing keys in `~/.verso/.env` so they’re picked up even when the service doesn’t inherit your shell env.
...
`VERSO_LOAD_SHELL_ENV=1`, `VERSO_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`verso models status` reports whether **shell env import** is enabled. “Shell env: off”
does **not** mean your env vars are missing - it just means Verso won’t load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won’t inherit your shell
environment. Fix by doing one of these:

1) Put the token in `~/.verso/.env`:
   ```
...
### Is there a way to make a team of Versos one CEO and many agents
...
### How do I completely reset Verso but keep it installed
...
```bash
verso reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `VERSO_PROFILE`), reset each state dir (defaults are `~/.verso-<profile>`).
- Dev reset: `verso gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), Verso skips the heartbeat run to save API calls.
...
No. Verso runs on **your own account**, so if you’re in the group, Verso can see it.
...
```bash
verso directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt Verso reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.verso/agents/<agentId>/sessions/`.
...
- Use `verso doctor` to spot stray workspaces and profile mismatches.
...
Verso’s default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-5`). If you omit the provider, Verso currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `verso models set ...` (updates just model config)
- `verso configure --section models` (interactive)
- edit `agents.defaults.model` in `~/.verso/verso.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `verso doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do Verso Flawd and Krill use for models

- **Verso + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-5`) - see [Anthropic](/providers/anthropic).
...
Yes. Verso ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
Fix options:
- Run `verso agents add <id>` and configure auth during the wizard.
...
### Why does verso gateway status show Config cli and Config service different

You’re editing one config file while the service is running another (often a `--profile` / `VERSO_STATE_DIR` mismatch).

Fix:
```bash
verso gateway install --force
```
Run that from the same `--profile` / environment you want the service to use.

### What does another gateway instance is already listening mean

Verso enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `verso gateway --port <port>`.

### How do I run Verso in remote mode client connects to a Gateway elsewhere
...
Notes:
- `verso gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
Facts (from code):
- The Control UI stores the token in browser localStorage key `verso.control.settings.v1`.
- The UI can import `?token=...` (and/or `?password=...`) once, then strips it from the URL.

Fix:
- Fastest: `verso dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
- If you don’t have a token yet: `verso doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...`.
- Set `gateway.auth.token` (or `VERSO_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token (or refresh with a one-time `?token=...` link).
- Still stuck? Run `verso status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `VERSO_CONFIG_PATH` (per‑instance config)
- `VERSO_STATE_DIR` (per‑instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):
- Use `verso --profile <name> …` per instance (auto-creates `~/.verso-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `verso --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.verso.*`, `verso-gateway-<profile>.service`, `Verso Gateway (<profile>)`).
...
```
verso tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/verso/verso-YYYY-MM-DD.log
```
...
- macOS: `$VERSO_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.verso/logs/...`; profiles use `~/.verso-<profile>/logs/...`)
- Linux: `journalctl --user -u verso-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "Verso Gateway (<profile>)" /V /FO LIST`
