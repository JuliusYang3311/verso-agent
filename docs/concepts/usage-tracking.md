...
- `/usage cost` in chats: local cost summary aggregated from Verso session logs.
- CLI: `verso status --usage` prints a full per-provider breakdown.
- CLI: `verso channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
