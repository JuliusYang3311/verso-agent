...
On the first turn of a new session, Verso injects the contents of these files directly into the agent context.

Blank files are skipped. Large files are trimmed and truncated with a marker so prompts stay lean (read the file for full content).

If a file is missing, Verso injects a single “missing file” marker line (and `verso setup` will create a safe default template).
...
Verso loads skills from three locations (workspace wins on name conflict):
- Bundled (shipped with the install)
- Managed/local: `~/.verso/skills`
...
Verso reuses pieces of the p-mono codebase (models/tools), but **session management, discovery, and tool wiring are Verso-owned**.
...
- `~/.verso/agents/<agentId>/sessions/<SessionId>.jsonl`

The session ID is stable and chosen by Verso.
