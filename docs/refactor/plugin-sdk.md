...
Delivery:
- Publish as `@verso/plugin-sdk` (or export from core under `verso/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `VersoPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: VersoConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: VersoConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: VersoConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: VersoConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: VersoConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: VersoConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@verso/plugin-sdk`.
- Add `api.runtime` to `VersoPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `versoRuntime: ">=2026.2.0"`).
