---
summary: "Research notes: offline memory system for Verso workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/verso) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Verso integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Verso-style workspace (`agents.defaults.workspace`, default `~/verso`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/verso` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Verso to see every session.
...
Recommendation: **deep integration in Verso**, but keep a separable core library.

### Why integrate into Verso?
- Verso already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `verso memory recall "…" --k 25 --since 30d`
  - `verso memory reflect --since 7d`
