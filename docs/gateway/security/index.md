...
If you run `--deep`, Verso also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.verso/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.verso/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.verso/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.verso/credentials/oauth.json`
...
`verso security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${VERSO_GATEWAY_PASSWORD}
```
...
Verso stores session transcripts on disk under `~/.verso/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.verso` (see the audit section below). If you need
...
Verso can refresh the skills list mid-session:
...
- If you install plugins from npm (`verso plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.verso/extensions/<pluginId>/` (or `$VERSO_STATE_DIR/extensions/<pluginId>/`).
  - Verso uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
verso pairing approve <channel> <code>
```
...
By default, Verso routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Verso has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.verso/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.verso or your logs.”
...
4. **Re-run audit**
   - `verso security audit --deep` and confirm the report is clean.

## Lessons Learned (The Hard Way)

### The `find ~` Incident 🦞

On Day 1, a friendly tester asked Verso to run `find ~` and share the output. Verso happily dumped the entire home directory structure to a group chat.
...
- `~/.verso/verso.json`: `600` (user read/write only)
- `~/.verso`: `700` (user only)

`verso doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `VERSO_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_verso-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `VERSO_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `verso doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `VERSO_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `VERSO_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Verso
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Verso verifies the identity by resolving the
...
- Verso will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.verso/` (or `$VERSO_STATE_DIR/`) may contain secrets or private data:

- `verso.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `verso status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@verso", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.verso/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `verso` profile).
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `verso gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `VERSO_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/verso/verso-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.verso/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Verso version
