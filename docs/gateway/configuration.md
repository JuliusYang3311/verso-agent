...
- Default log file: `/tmp/verso/verso-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/verso/verso.log`.
...
- The legacy single-account Baileys auth dir is migrated by `verso doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@verso", "verso", "verso"] } }
    ]
  }
}
```
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Verso uses
...
- Shortcut: `verso onboard --auth-choice opencode-zen`.
...
Shortcut: `verso onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `verso onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `verso onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `verso onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `verso onboard --auth-choice minimax-api`.
...
- Override the agent config root with `VERSO_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.verso/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.verso/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.verso/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Verso stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.verso/skills` (workspace skills
...
from `~/.verso/extensions`, `<workspace>/.verso/extensions`, plus any
...
### `browser` (verso-managed browser)

Verso can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for verso and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Verso.app menubar, or `verso gateway`).
...
- When a connection comes from one of these IPs, Verso uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `verso gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `VERSO_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `VERSO_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Verso
...
macOS app behavior:
- Verso.app watches `~/.verso/verso.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.verso/verso.json` (or `VERSO_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.verso/verso.json` (or `VERSO_CONFIG_PATH`)
...
- `VERSO_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `verso --dev …` → uses `~/.verso-dev` + shifts ports from base `19001`
- `verso --profile <name> …` → uses `~/.verso-<name>` (port via config/env/flags)
...
Gmail helper config (used by `verso webhooks gmail setup` / `run`):
...
- Set `VERSO_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Verso defaults `serve.path` to `/` so
...
Default root: `~/verso/canvas`  
Default port: `18793` (chosen to avoid the verso browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__verso/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__verso__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.verso/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.verso/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.verso/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_verso-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_verso-bridge._tcp` under `~/.verso/dns/` using the standard discovery domain `verso.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `verso.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `verso.internal` via that server
