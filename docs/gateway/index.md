...
[Service]
ExecStart=/usr/local/bin/verso gateway --port 18789
Restart=always
RestartSec=5
Environment=VERSO_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/verso-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now verso-gateway[-<profile>].service
```
...
## CLI helpers
- `verso gateway health|status` — request health/status over the Gateway WS.
- `verso message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `verso agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `verso gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `verso gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `verso gateway` and the legacy TCP control port.
