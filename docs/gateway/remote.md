...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Verso runs”).
...
With the tunnel up:
- `verso health` and `verso status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `verso gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`VERSO_GATEWAY_PORT`).
