---
summary: "Quick troubleshooting guide for common Verso failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Verso misbehaves, here's how to fix it.
...
|---|---|---|
| `verso status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `verso status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `verso status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `verso gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `verso channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `verso gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `verso logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `verso status --all` (it redacts tokens). If you paste `verso status`, consider setting `VERSO_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  verso models auth setup-token --provider anthropic
  ```
...
```bash
verso models auth paste-token --provider anthropic
verso models status
```
...
**Logs:**
- Preferred: `verso logs --follow`
- File logs (always): `/tmp/verso/verso-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$VERSO_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u verso-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  verso config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  verso config set gateway.mode remote
  verso config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `verso setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.verso/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `verso doctor`
...
- then run `verso sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `verso gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `verso gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `verso gateway status` and `verso doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `VERSO_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `verso gateway status` shows a config mismatch**
...
- Fix: rerun `verso gateway install --force` from the same `--profile` / `VERSO_STATE_DIR` you want the service to use.

**If `verso gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `verso doctor` to update it (or `verso gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `VERSO_GATEWAY_TOKEN`) and restart the service.

**If `verso gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/verso` on disk.
...
Symptoms: `pwd` or file tools show `~/.verso/sandboxes/...` even though you
...
Verso intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `verso models list` or
  `verso models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${VERSO_CONFIG_PATH:-$HOME/.verso/verso.json}"
```
...
tail -f "$(ls -t /tmp/verso/verso-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
verso logs --follow | grep "pairing request"
```
...
- ❌ `@verso` + image
- ✅ `@verso check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.verso/agents/<agentId>/sessions/
```
...
verso logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
verso gateway --verbose
```

If you’re logged out / unlinked:

```bash
verso channels logout
trash "${VERSO_STATE_DIR:-$HOME/.verso}/credentials" # if logout can't cleanly remove everything
verso channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/verso/verso-*.log | head -1)" | tail -20
```

### High Memory Usage

Verso keeps conversation history in memory.
...
Verso now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `verso doctor` reports every invalid entry.
- `verso doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `verso logs`, `verso health`, `verso status`, `verso gateway status`, and `verso gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/verso/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `verso models status` (CLI).
...
5) Run `verso channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Verso scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/verso/verso/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
verso gateway status
verso gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.verso.* still works)
```
...
Ensure the global `verso` CLI is installed and matches the app version:
```bash
verso --version
npm install -g verso@<version>
```
...
#   ${VERSO_CONFIG_PATH:-$HOME/.verso/verso.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
verso gateway --verbose
verso channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/verso/verso-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$VERSO_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.verso/logs/...`; profiles use `~/.verso-<profile>/logs/...`)<br />Linux: `journalctl --user -u verso-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Verso Gateway (<profile>)" /V /FO LIST` |
| Session files | `$VERSO_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$VERSO_STATE_DIR/media/` |
| Credentials | `$VERSO_STATE_DIR/credentials/` |
...
trash "${VERSO_STATE_DIR:-$HOME/.verso}"
verso channels login         # re-pair WhatsApp
verso gateway restart           # or: verso gateway
```
...
1. Check logs first: `/tmp/verso/` (default: `verso-YYYY-MM-DD.log`, or your configured `logging.file`)
