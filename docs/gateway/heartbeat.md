...
- During heartbeat runs, Verso treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `verso sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Verso skips the heartbeat run entirely (no model call).
...
```bash
verso system event --text "Check for urgent follow-ups" --mode now
```
