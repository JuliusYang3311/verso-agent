...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Verso would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Verso would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/verso-alfred/
...
Workspace: /Users/pascal/verso-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.verso/logs/gateway.log | grep broadcast
```
