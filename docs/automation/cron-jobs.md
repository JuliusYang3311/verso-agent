...
- Jobs persist under `~/.verso/cron/` so restarts don’t lose schedules.
...
If `delivery` is omitted for isolated jobs, Verso defaults to `announce`.
...
- Job store: `~/.verso/cron/jobs.json` (Gateway-managed JSON).
- # Run history: `~/.verso/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
verso cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
verso cron edit <jobId> --agent ops
verso cron edit <jobId> --clear-agent
```
...
```bash
verso system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`verso system event`](/cli/system).
...
- # Check cron is enabled: `cron.enabled` and `VERSO_SKIP_CRON`.
...
- Verso applies exponential retry backoff for recurring jobs after consecutive errors:
