...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `verso sandbox explain`.
...
Legacy `agent.*` configs are migrated by `verso doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=verso.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${VERSO_STATE_DIR:-$HOME/.verso}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
