...
- A public registry for Verso skills.
...
4. Users browse, download, and install skills in Verso.
...
If you want to add new capabilities to your Verso agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
By default, the CLI installs skills into `./skills` under your current working directory. If a Verso workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Verso loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.openclaw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Verso how to perform a
...
Interested in becoming a moderator? Ask in the Verso Discord and contact a
