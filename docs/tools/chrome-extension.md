---
summary: "Chrome extension: let Verso drive your existing Chrome tab"
read_when:
...
The Verso Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate verso-managed Chrome profile.
...
Verso then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
- Open the tab you want Verso to control.
...
- `ON`: attached; Verso can drive that tab.
...
Debugging: `verso sandbox explain`
...
`verso browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `verso browser extension install` first to copy the extension to a stable location under your Verso state directory.
...
- **This is not isolated** like the dedicated verso-managed profile.
