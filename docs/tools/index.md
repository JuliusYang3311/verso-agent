---
summary: "Agent tool surface for Verso (browser, canvas, nodes, message, cron) replacing legacy `verso-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `verso-*` skills
---

# Tools (Verso)

Verso exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `verso-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `verso.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Verso logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`verso node run`).
...
Notes:
- Requires a Brave API key (recommended: `verso configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated verso browser.
...
- Quick smoke: `verso nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `verso gateway` restart in-place)
