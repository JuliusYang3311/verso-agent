...
- # **gateway host** → `verso` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.verso/exec-approvals.sock",
...
`~/.verso/exec-approvals.json` directly.

CLI: `verso approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
