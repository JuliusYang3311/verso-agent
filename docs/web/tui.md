...
```bash
verso tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `verso tui --deliver`
...
- Check the Gateway logs: `verso logs --follow`.
- Confirm the agent can run: `verso status` and `verso models status`.
...
- No agents in picker: check `verso agents list` and your routing config.
