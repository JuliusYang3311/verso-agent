...
Verso integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Verso owns lifecycle when `channels.signal.autoStart=true`.

See [Signal](/channels/signal) for setup and endpoints.

## Pattern B: stdio child process (imsg)

- # Verso spawns `imsg rpc` as a child process.
- Verso owns lifecycle when `channels.signal.autoStart=true`.
...
- Verso spawns `imsg rpc` as a child process (legacy iMessage integration).
  > > > > > > > upstream/main
