...
read_when:
  - You want to place an outbound voice call from Verso
...
Voice calls for Verso via a plugin. Supports outbound notifications and
...
- Use `verso voicecall ...` or the `voice_call` tool
...
```bash
verso plugins install ./extensions/voice-call
