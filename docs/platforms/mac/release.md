---
summary: "Verso macOS release checklist (Sparkle feed, packaging, signing)"
read_when:
...
- Notary credentials (keychain profile or API key) for `xcrun notarytool` if you want Gatekeeper-safe DMG/zip distribution.
  - We use a Keychain profile named `verso-notary`, created from App Store Connect API key env vars in your shell profile:
    - `APP_STORE_CONNECT_API_KEY_P8`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`
    - `echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/verso-notary.p8`
    - `xcrun notarytool store-credentials "verso-notary" --key /tmp/verso-notary.p8 --key-id "$APP_STORE_CONNECT_KEY_ID" --issuer "$APP_STORE_CONNECT_ISSUER_ID"`
...
ditto -c -k --sequesterRsrc --keepParent dist/Verso.app dist/Verso-2026.1.29.zip

# Optional: also build a styled DMG for humans (drag to /Applications)
scripts/create-dmg.sh dist/Verso.app dist/Verso-2026.1.29.dmg

# Recommended: build + notarize/staple zip + DMG
# First, create a keychain profile once:
#   xcrun notarytool store-credentials "verso-notary" \
...
ditto -c -k --keepParent apps/macos/.build/release/Verso.app.dSYM dist/Verso-2026.1.29.dSYM.zip
```
