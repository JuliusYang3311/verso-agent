...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (`verso.internal.`), **or**
...
The Gateway advertises `_verso._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (recommended domain: `verso.internal.`) and Tailscale split DNS.
...
```bash
verso nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18793/__verso__/canvas/"}'
```
...
- Pairing prompt never appears: run `verso nodes pending` and approve manually.
