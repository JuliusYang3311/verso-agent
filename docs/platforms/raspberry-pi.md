---
summary: "Verso on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Verso on a Raspberry Pi
  - Running Verso on ARM devices
  - Building a cheap always-on personal AI
---

# Verso on Raspberry Pi

## Goal

Run a persistent, always-on Verso Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Verso features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled verso

# Enable if not
sudo systemctl enable verso

# Start on boot
sudo systemctl start verso
```
...
```bash
# Check logs
journalctl -u verso --no-pager -n 100

# Common fix: rebuild
cd ~/verso  # if using hackable install
npm run build
sudo systemctl restart verso
```
