...
Deploy Verso on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy Verso](https://northflank.com/stacks/deploy-openclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy Verso now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the Verso template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the Verso service.
8. Open the public Verso URL and complete setup at `/setup`.
9. Open the Control UI at `/openclaw`.

## What you get

- Hosted Verso Gateway + Control UI
