---
summary: "Optional Docker-based setup and onboarding for Verso"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Verso on a host without local installs.
...
```bash
docker build -t verso:local -f Dockerfile .
docker compose run --rm verso-cli onboard
...
`VERSO_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`verso-gateway` and `verso-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export VERSO_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `VERSO_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `VERSO_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export VERSO_HOME_VOLUME="verso_home"
./docker-setup.sh
```
...
libraries), set `VERSO_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export VERSO_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `VERSO_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
docker compose run --rm verso-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm verso-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm verso-cli channels add --channel discord --token "<token>"
```
...
```bash
docker compose exec verso-gateway node dist/index.js health --token "$VERSO_GATEWAY_TOKEN"
```
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.verso/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Verso auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `verso sandbox recreate ...` command.
...
This builds `verso-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
```json5
{
  agents: { defaults: { sandbox: { docker: { image: "verso-sandbox-common:bookworm-slim" } } } },
}
```
...
This builds `verso-sandbox-browser:bookworm-slim` using
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-verso-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-verso-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-verso-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/verso/verso/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: Verso runs commands with `sh -lc` (login shell), which
