...
- If upgrading an existing install: runs `verso doctor --non-interactive` (best effort).
- For git installs: runs `verso doctor --non-interactive` after install/update (best effort).
...
In non-interactive contexts (no TTY / `--no-prompt`), you must pass `--install-method git|npm` (or set `VERSO_INSTALL_METHOD`), otherwise the script exits with code `2`.
...
This script installs `verso` into a prefix (default: `~/.verso`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
...
- Choose install method:
  - `npm` (default): `npm install -g verso@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `verso doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://molt.bot/install.ps1 | iex -InstallMethod git -GitDir "C:\\verso"
```
