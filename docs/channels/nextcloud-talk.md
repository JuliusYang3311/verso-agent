...
```bash
verso plugins install @verso/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
verso plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Verso" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `verso pairing list nextcloud-talk`
  - `verso pairing approve nextcloud-talk <CODE>`
