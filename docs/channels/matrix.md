...
Matrix is an open, decentralized messaging protocol. Verso connects as a Matrix **user**
...
```bash
verso plugins install @verso/matrix
```

Local checkout (when running from a git repo):

```bash
verso plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `verso plugins install @verso/matrix`
   - From a local checkout: `verso plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Verso calls the same
     login endpoint, stores the access token in `~/.verso/credentials/matrix/credentials.json`,
...
- On first connection, Verso requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Verso logs a warning.
...
`~/.verso/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `verso pairing list matrix`
  - `verso pairing approve matrix <CODE>`
...
- On startup, Verso resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
