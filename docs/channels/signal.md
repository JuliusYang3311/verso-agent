...
This skips auto-spawn and the startup wait inside Verso. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `verso pairing list signal`
  - `verso pairing approve signal <CODE>`
...
- **Typing indicators**: Verso sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Verso forwards read receipts for allowed DMs.
