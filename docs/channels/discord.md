...
Discord uses numeric ids everywhere; Verso config prefers ids.
...
- First: run `verso doctor` and `verso channels status --probe` (actionable warnings + quick audits).
...
- On startup, Verso resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror Verso’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Verso enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`verso gateway --force`) after confirming no other processes own the Discord session.
