...
read_when:
  - You want Verso to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Verso to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`verso onboard`) and `verso channels add` list optional channel plugins.
...
```bash
verso plugins install @verso/nostr
```

Use a local checkout (dev workflows):

```bash
verso plugins install --link <path-to-verso>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "verso@example.com",
        "lud16": "verso@example.com"
      }
    }
  }
}
