...
Private chats can include `message_thread_id` in some edge cases. Verso keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2) Run `verso logs --follow` and look for `from.id`.
...
Verso defaults to audio files for backward compatibility.
...
Verso supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, Verso handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, Verso caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.verso/telegram/sticker-cache.json`
...
Verso uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, Verso:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by Verso)
...
- Example: `verso message send --channel telegram --target 123456789 --message "hi"`.
...
- `verso channels status` shows a warning when config expects unmentioned group messages.
- `verso channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `verso logs --follow` (look for "skipping group message")
...
- Upgrade to a Verso build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
