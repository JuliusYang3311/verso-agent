...
Tlon is a decentralized messenger built on Urbit. Verso connects to your Urbit ship and can
...
```bash
verso plugins install @verso/tlon
```

Local checkout (when running from a git repo):

```bash
verso plugins install ./extensions/tlon
```
...
Use these with `verso message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Verso replies in-thread.
