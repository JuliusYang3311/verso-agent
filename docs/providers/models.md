---
summary: "Model providers (LLMs) supported by Verso"
read_when:
...
Verso can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `verso onboard`).
