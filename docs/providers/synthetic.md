---
summary: "Use Synthetic's Anthropic-compatible API in Verso"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Verso registers it as the
...
```bash
verso onboard --auth-choice synthetic-api-key
```
...
Note: Verso's Anthropic client appends `/v1` to the base URL, so use
