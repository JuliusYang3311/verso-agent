...
Verso includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
...
Paste the token into Verso (wizard: **Anthropic token (paste setup-token)**), or run it on the gateway host:
...
verso onboard --auth-choice setup-token
```
...
- Generate the setup-token with `claude setup-token` and paste it, or run `verso models auth setup-token` on the gateway host.
...
- If the Claude CLI login lives on a different machine, use
  `verso models auth paste-token --provider anthropic` on the gateway host.

**No API key found for provider "anthropic"**
- Auth is **per agent**. New agents don’t inherit the main agent’s keys.
- Re-run onboarding for that agent, or paste a setup-token / API key on the
  gateway host, then verify with `verso models status`.

**No credentials found for profile `anthropic:default`**
- Run `verso models status` to see which auth profile is active.
- Re-run onboarding, or paste a setup-token / API key for that profile.

**No available auth profile (all in cooldown/unavailable)**
- Check `verso models status --json` for `auth.unusableProfiles`.
