...
- Before that (Bun path), `verso status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch verso.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node verso.mjs status` works on Node 25.
