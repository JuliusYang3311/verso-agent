...
This document describes how the Verso ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`verso acp` exposes an ACP agent over stdio and forwards prompts to a running
Verso Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Verso Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `verso acp` over stdio.

Example config:

```bash
verso config set gateway.remote.url wss://gateway-host:18789
verso config set gateway.remote.token <token>
```

Example run:

```bash
verso acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
verso acp --session agent:main:main
verso acp --session agent:design:main
verso acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Verso ACP” to start a thread.

## Execution Model

- ACP client spawns `verso acp` and speaks ACP messages over stdio.
...
```bash
verso acp --session agent:main:main
verso acp --session-label "support inbox"
verso acp --reset-session
```
...
`verso acp` resolves the Gateway URL and auth from CLI flags or config:
