...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when VERSO_NIX_MODE is not set", async () => {
      await withEnvOverride({ VERSO_NIX_MODE: undefined }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when VERSO_NIX_MODE is empty", async () => {
      await withEnvOverride({ VERSO_NIX_MODE: "" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when VERSO_NIX_MODE is not '1'", async () => {
      await withEnvOverride({ VERSO_NIX_MODE: "true" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is true when VERSO_NIX_MODE=1", async () => {
      await withEnvOverride({ VERSO_NIX_MODE: "1" }, async () => {
        const { isNixMode } = await import("./config.js");
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("CONFIG_PATH defaults to ~/.verso/verso.json when env not set", async () => {
      await withEnvOverride(
        {
        ...
        async () => {
          ...
          expect(CONFIG_PATH).toMatch(/\.verso[\\/]verso\.json$/);
        },
      );
    });
    ...
    it("CONFIG_PATH prefers VERSO_CONFIG_PATH over legacy override", async () => {
      await withEnvOverride(
        {
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/new/verso.json"));
        },
      );
    });
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(path.resolve("/custom/state"), "verso.json"));
        },
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        ...
        await fs.writeFile(
          path.join(pluginDir, "verso.plugin.json"),
          JSON.stringify(
            {
              id: "demo-plugin",
          ...
          JSON.stringify(
            {
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.verso/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".verso", "agents", "main"));
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".verso", "credentials", "wa-personal"),
        );
      });
    });
  });
  ...
  describe("U6: gateway port resolution", () => {
    ...
    it("falls back to config when env is invalid", async () => {
      await withEnvOverride({ VERSO_GATEWAY_PORT: "nope" }, async () => {
        const { resolveGatewayPort } = await import("./config.js");
