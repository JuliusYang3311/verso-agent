...
import type { VersoConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: VersoConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: VersoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as VersoConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as VersoConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as VersoConfig)),
    ),
  };
}
