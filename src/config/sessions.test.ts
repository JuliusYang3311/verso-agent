...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-verso",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-verso#general");
  });
  ...
  it("derives session transcripts dir from VERSO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { VERSO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.VERSO_STATE_DIR = "/custom/state";
    try {
