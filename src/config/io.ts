...
import type { VersoConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
function coerceConfig(value: unknown): VersoConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as VersoConfig;
}
...
function stampConfigVersion(cfg: VersoConfig): VersoConfig {
...
function warnIfConfigFromFuture(cfg: VersoConfig, logger: Pick<typeof console, "warn">): void {
...
function applyConfigEnv(cfg: VersoConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): VersoConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as VersoConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: VersoConfig) {
    clearConfigCache();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.VERSO_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): VersoConfig {
...
export async function writeConfigFile(cfg: VersoConfig): Promise<void> {
  clearConfigCache();
