...
describe("oauth paths", () => {
  it("prefers VERSO_OAUTH_DIR over VERSO_STATE_DIR", () => {
    const env = {
      VERSO_OAUTH_DIR: "/custom/oauth",
  ...
  it("derives oauth path from VERSO_STATE_DIR when unset", () => {
    const env = {
      VERSO_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("state + config path candidates", () => {
  ...
  it("prefers ~/.verso when it exists and legacy dir is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "verso-state-"));
    try {
      const newDir = path.join(root, ".verso");
  ...
  it("CONFIG_PATH prefers existing legacy filename when present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "verso-config-"));
    ...
    try {
      ...
      const legacyPath = path.join(legacyDir, "verso.json");
  ...
  it("respects state dir overrides when config is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "verso-config-override-"));
    try {
      const legacyDir = path.join(root, ".verso");
      await fs.mkdir(legacyDir, { recursive: true });
      const legacyConfig = path.join(legacyDir, "verso.json");
      await fs.writeFile(legacyConfig, "{}", "utf-8");

      const overrideDir = path.join(root, "override");
      const env = { VERSO_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const resolved = resolveConfigPath(env, overrideDir, () => root);
      expect(resolved).toBe(path.join(overrideDir, "verso.json"));
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
