...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("verso skills list")}\` to see all skills.`
      : "No skills found.";
    return appendVersoHubHint(message, opts.json);
  }
  ...
  return appendVersoHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendVersoHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("verso skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.verso.ai/cli/skills")}\n`,
    );
