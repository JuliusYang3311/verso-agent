...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("verso node install"), formatCliCommand("verso node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("verso node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    VERSO_LOG_PREFIX: baseEnv.VERSO_LOG_PREFIX ?? "node",
