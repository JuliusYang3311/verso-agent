...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Verso Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(verso completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(verso completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "verso") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c verso -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'verso gateway' and not 'verso other gateway', we need to check the sequence.
