...
export function applyCliProfileEnv(params: {
  ...
  env.VERSO_PROFILE = profile;

  const stateDir = env.VERSO_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.VERSO_STATE_DIR?.trim()) env.VERSO_STATE_DIR = stateDir;

  if (!env.VERSO_CONFIG_PATH?.trim()) {
    env.VERSO_CONFIG_PATH = path.join(stateDir, "verso.json");
  }

  if (profile === "dev" && !env.VERSO_GATEWAY_PORT?.trim()) {
    env.VERSO_GATEWAY_PORT = "19001";
  }
}
