...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*verso-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "verso";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "verso");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "verso-gw");
  ...
  records.push(`_verso-gw._tcp IN PTR ${instanceLabel}._verso-gw._tcp`);
  records.push(`${instanceLabel}._verso-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._verso-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; verso-content-hash: ${contentHash}\n${contentBody}`;
}
