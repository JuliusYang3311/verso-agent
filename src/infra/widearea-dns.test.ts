...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a verso.internal zone with gateway PTR/SRV/TXT records", () => {
    ...
    expect(txt).toContain(`studio-london._verso-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Verso)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/verso`);
  });
