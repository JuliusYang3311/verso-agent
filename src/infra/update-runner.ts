...
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "verso", "openclaw"]);
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "verso-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "verso doctor",
        managerScriptArgs(manager, "verso", ["doctor", "--non-interactive"]),
        gitRoot,
        { VERSO_UPDATE_IN_PROGRESS: "1" },
      ),
    );
