...
const prevBundledDir = process.env.VERSO_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `verso-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "verso.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.VERSO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadVersoPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.VERSO_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadVersoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
