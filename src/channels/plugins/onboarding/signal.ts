...
function setSignalDmPolicy(cfg: VersoConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function setSignalAllowFrom(cfg: VersoConfig, accountId: string, allowFrom: string[]): VersoConfig {
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "Verso"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("verso gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
