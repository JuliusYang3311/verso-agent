...
function setSlackDmPolicy(cfg: VersoConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Verso";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Verso`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: VersoConfig, allowFrom: string[]): VersoConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
