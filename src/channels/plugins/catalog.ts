...
import type { VersoPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["VERSO_PLUGIN_CATALOG_PATHS", "VERSO_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<VersoPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageVerso?: VersoPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageVerso;
  if (!manifest?.channel) {
    return null;
  }
