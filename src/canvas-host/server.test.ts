...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("versoCanvasA2UIAction");
    expect(out).toContain("versoSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("versoSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.versoA2UI = {};", "utf8");
      createdBundle = true;
    }
