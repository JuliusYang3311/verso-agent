...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.versoCanvasA2UIAction ||
        window.webkit.messageHandlers.versoCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.versoCanvasA2UIAction &&
        typeof window.versoCanvasA2UIAction.postMessage === "function") ||
      (window.versoCanvasA2UIAction &&
        typeof window.versoCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.versoSendUserAction === "function" ? window.versoSendUserAction : undefined;
  if (!window.versoSendUserAction && legacySend) {
    window.versoSendUserAction = legacySend;
  }
  if (!window.versoSendUserAction && typeof window.versoSendUserAction === "function") {
    window.versoSendUserAction = window.versoSendUserAction;
  }
  const hasHelper = () =>
    typeof window.versoSendUserAction === "function" ||
    typeof window.versoSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("verso:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Verso node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.versoSendUserAction === "function"
        ? window.versoSendUserAction
        : window.versoSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.VERSO_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "verso", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Verso Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
