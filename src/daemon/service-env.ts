...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.VERSO_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const _stateDir = env.OPENCLAW_STATE_DIR;
  const _configPath = env.OPENCLAW_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    VERSO_PROFILE: profile,
    VERSO_STATE_DIR: env.VERSO_STATE_DIR,
    VERSO_CONFIG_PATH: env.VERSO_CONFIG_PATH,
    VERSO_GATEWAY_PORT: String(port),
    VERSO_GATEWAY_TOKEN: token,
    VERSO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    VERSO_SYSTEMD_UNIT: systemdUnit,
    VERSO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    VERSO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    VERSO_STATE_DIR: env.VERSO_STATE_DIR,
    VERSO_CONFIG_PATH: env.VERSO_CONFIG_PATH,
    VERSO_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    VERSO_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    VERSO_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    VERSO_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    VERSO_LOG_PREFIX: "node",
    VERSO_SERVICE_MARKER: NODE_SERVICE_MARKER,
    VERSO_SERVICE_KIND: NODE_SERVICE_KIND,
    VERSO_SERVICE_VERSION: VERSION,
  };
}
