...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.VERSO_GATEWAY_PASSWORD,
    disableConfigCache: process.env.VERSO_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("VERSO_STATE_DIR", prev.stateDir);
  restoreEnvVar("VERSO_CONFIG_PATH", prev.configPath);
  restoreEnvVar("VERSO_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("VERSO_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("VERSO_SKIP_CRON", prev.skipCron);
  restoreEnvVar("VERSO_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("VERSO_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("VERSO_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("VERSO_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.VERSO_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "verso.json");
  process.env.HOME = tempHome;
  process.env.VERSO_STATE_DIR = tempHome;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("verso-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("verso-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("verso-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("verso-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("verso-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("verso-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
