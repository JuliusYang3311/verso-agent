...
function hasConfigGatewayCreds(cfg: VersoConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const lines = [
    ...
    envToken ? "- `VERSO_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword ? "- `VERSO_GATEWAY_PASSWORD` is set; it overrides config passwords." : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv VERSO_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv VERSO_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
