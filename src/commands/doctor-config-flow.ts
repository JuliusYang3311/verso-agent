...
function stripUnknownConfigKeys(config: VersoConfig): {
  config: VersoConfig;
  removed: string[];
} {
  const parsed = VersoSchema.safeParse(config);
  if (parsed.success) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "verso.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "verso.json");
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("verso doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "verso doctor --fix" to remove these keys.');
    }
  }
