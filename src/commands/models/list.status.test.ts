...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/verso-agent/auth-profiles.json"),
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveVersoAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/verso-agent/auth-profiles.json");
