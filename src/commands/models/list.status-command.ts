...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveVersoAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("verso models auth setup-token")}\` or \`${formatCliCommand("verso configure")}\`.`
          : `Run \`${formatCliCommand("verso configure")}\` or set an API key env var.`;
