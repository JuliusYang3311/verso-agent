...
import { type VersoConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function updateConfig(
  mutator: (cfg: VersoConfig) => VersoConfig,
): Promise<VersoConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: VersoConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: VersoConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("verso agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
