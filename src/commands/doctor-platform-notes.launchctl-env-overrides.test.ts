...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "VERSO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("VERSO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv VERSO_GATEWAY_TOKEN");
    expect(message).not.toContain("VERSO_GATEWAY_PASSWORD");
  });
