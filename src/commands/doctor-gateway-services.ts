...
async function cleanupLegacyLaunchdService(params: {
  ...
  if (loaded) {
    note(`Verso ${service.label} already ${service.loadedText}.`, "Gateway");
    return;
  }

  const install = await prompter.confirmSkipInNonInteractive({
    message: "Install Verso gateway service now?",
    initialValue: true,
  });
  if (!install) {
    return;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Verso gateway next.");
      }
    }
  }
