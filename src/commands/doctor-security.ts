...
import type { VersoConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: VersoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("verso security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("verso configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("verso config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("verso doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("verso config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
