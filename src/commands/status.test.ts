...
beforeAll(() => {
  previousProfile = process.env.VERSO_PROFILE;
  process.env.VERSO_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.VERSO_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/verso-root.js", () => ({
  resolveVersoPackageRoot: vi.fn().mockResolvedValue("/tmp/verso"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/verso/pnpm-lock.yaml",
      markerPath: "/tmp/verso/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Verso status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("verso status --all") ||
          l.includes("verso --profile isolated status --all") ||
          l.includes("verso status --all") ||
          l.includes("verso --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.VERSO_GATEWAY_TOKEN = "abcd1234";
    try {
