...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Verso status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("verso security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("verso security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.verso.ai/faq");
  runtime.log("Troubleshooting: https://docs.verso.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("verso status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("verso logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("verso status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("verso gateway probe")}`);
  }
}
