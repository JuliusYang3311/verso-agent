...
const requireAgentDir = () => {
  const agentDir = process.env.VERSO_AGENT_DIR;
  if (!agentDir) throw new Error("VERSO_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.VERSO_OAUTH_DIR;
  });

  it("writes auth-profiles.json under VERSO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-oauth-"));
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to VERSO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-minimax-"));
    process.env.VERSO_STATE_DIR = tempStateDir;
    process.env.VERSO_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.VERSO_AGENT_DIR;
