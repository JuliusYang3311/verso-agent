...
export function listAgentEntries(cfg: VersoConfig): AgentEntry[] {
...
function resolveAgentName(cfg: VersoConfig, agentId: string) {
...
function resolveAgentModel(cfg: VersoConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: VersoConfig): AgentSummary[] {
