...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.VERSO_SKIP_BROWSER_CONTROL_SERVER,
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "verso-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.VERSO_STATE_DIR = prev.stateDir;
    ...
    process.env.VERSO_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.VERSO_GATEWAY_TOKEN = prev.token;
    process.env.VERSO_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "verso");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.VERSO_STATE_DIR = stateDir;
    process.env.VERSO_CONFIG_PATH = path.join(stateDir, "verso.json");
