...
const SEARCH_TOOL = "https://docs.molt.bot/mcp.SearchVerso";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.verso.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('verso docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.molt.bot/");
      runtime.log(`Search: ${formatCliCommand('verso docs "your query"')}`);
    }
    return;
  }
