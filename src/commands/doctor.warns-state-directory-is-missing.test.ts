...
beforeEach(() => {
  ...
  resolveVersoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.VERSO_STATE_DIR;
  originalUpdateInProgress = process.env.VERSO_UPDATE_IN_PROGRESS;
  process.env.VERSO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "verso-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.VERSO_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.VERSO_UPDATE_IN_PROGRESS;
  } else {
    process.env.VERSO_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveVersoPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadVersoPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/verso-root.js", () => ({
  resolveVersoPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "verso-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.VERSO_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when VERSO_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/verso.json",
      exists: true,
      raw: "{}",
    ...
    process.env.VERSO_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
