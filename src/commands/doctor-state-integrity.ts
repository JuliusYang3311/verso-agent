...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidate = path.resolve(root, entry.name, ".verso");
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".verso");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.verso out of git; it contains credentials and session history.",
