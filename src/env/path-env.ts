...
import { resolveBrewPathDirs } from "../infra/brew.js";
...
function candidateBinDirs(opts: EnsureVersoPathOpts): string[] {
  ...
  // Bundled macOS app: `verso` lives next to the executable (process.execPath).
  try {
...
/**
 * Best-effort PATH bootstrap so skills that require the `verso` CLI can run
