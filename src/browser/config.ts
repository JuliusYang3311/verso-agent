import type { BrowserConfig, BrowserProfileConfig, VersoConfig } from "../config/config.js";
import {
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) return DEFAULT_VERSO_BROWSER_COLOR;
  const normalized = value.startsWith("#") ? value : `#${value}`;
  if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_VERSO_BROWSER_COLOR;
 ...
 *
 * Note: this is a Verso browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for verso CDP).
