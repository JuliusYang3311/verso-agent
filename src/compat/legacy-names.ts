export const PROJECT_NAME = "verso" as const;

export const LEGACY_PROJECT_NAME = "openclaw" as const;
export const LEGACY_PROJECT_NAMES = [LEGACY_PROJECT_NAME] as const;

export const MANIFEST_KEY = PROJECT_NAME;

export const LEGACY_MANIFEST_KEY = LEGACY_PROJECT_NAME;
export const LEGACY_MANIFEST_KEYS = LEGACY_PROJECT_NAMES;

export const LEGACY_PLUGIN_MANIFEST_FILENAMES = [] as const;

export const LEGACY_CANVAS_HANDLER_NAMES = [] as const;

export const MACOS_APP_SOURCES_DIR = "apps/macos/Sources/Verso" as const;

export const LEGACY_MACOS_APP_SOURCES_DIRS = ["apps/macos/Sources/OpenClaw"] as const;
