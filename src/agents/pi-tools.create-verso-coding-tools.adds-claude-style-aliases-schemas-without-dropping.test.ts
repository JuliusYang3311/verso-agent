...
import { __testing, createVersoCodingTools } from "./pi-tools.js";
...
describe("createVersoCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createVersoCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-sbx-"));
