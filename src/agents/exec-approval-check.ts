/**
 * Exec approval and remote execution logic for node and gateway hosts.
 *
 * Extracted from bash-tools.exec.ts to reduce module size.
 */
import type { AgentToolResult } from "@mariozechner/pi-agent-core";
import crypto from "node:crypto";
import type { ExecProcessHandle, ExecToolDetails } from "./exec-run-process.js";
import {
  type ExecAsk,
...
export type NodeExecParams = {
  ...
  defaultsSessionKey?: string;
  notifySessionKey?: string;
  approvalRunningNoticeMs: number;
  warnings: string[];
};

export async function executeOnNode(
...
// ──────────────────────────────────────────────────────────
// Gateway approval check + execution
// ──────────────────────────────────────────────────────────

export type GatewayApprovalParams = {
  command: string;
  workdir: string;
  env: Record<string, string>;
  paramTimeout?: number;
  paramPty?: boolean;
  sandbox?: import("./bash-tools.shared.js").BashSandboxConfig;
  defaultTimeoutSec: number;
  security: ExecSecurity;
  ask: ExecAsk;
  agentId?: string;
...
/**
 * Result from gateway approval check.
 *
 * - `"handled"` means the approval was required and a pending-approval result was returned.
 * - `"denied"` is thrown as an Error.
 * - `"passed"` means the command may proceed to local execution.
 */
export type GatewayApprovalOutcome =
  | { action: "handled"; result: AgentToolResult<ExecToolDetails> }
  | { action: "passed" };

export async function checkGatewayApproval(
  params: GatewayApprovalParams,
): Promise<GatewayApprovalOutcome> {
  const { security, ask, agentId, workdir, env, warnings } = params;
  const approvals = resolveExecApprovals(agentId, { security, ask });
  const hostSecurity = minSecurity(security, approvals.agent.security);
  const hostAsk = maxAsk(ask, approvals.agent.ask);
