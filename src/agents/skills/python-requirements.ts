...
function extractPkgName(req: string): string {
...
function parsePyprojectDeps(filePath: string): string[] {
  if (!existsSync(filePath)) {
    return [];
  }
  const content = readFileSync(filePath, "utf-8");
...
export function collectPythonRequirements(params: {
  ...
  const seen = new Map<string, string>(); // normalized name → full requirement line
 ...
 */
export function hasPythonSkills(entries: SkillEntry[]): boolean {
