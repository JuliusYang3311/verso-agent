...
type VenvEntry = { path: string; createdAt: number };

const ACTIVE_VENVS = new Map<string, VenvEntry>();
const MAX_VENVS = 20;
...
export function getSessionVenvPath(sessionKey: string): string | null {
  const entry = ACTIVE_VENVS.get(sessionKey);
  return entry?.path ?? null;
}

/** Destroy the venv for a session and clean up disk + registry. */
export async function destroySessionVenv(sessionKey: string): Promise<void> {
  const entry = ACTIVE_VENVS.get(sessionKey);
  if (!entry) {
    return;
  }
...
export function reapStaleVenvs(): void {
  const now = Date.now();
...
function registerVenv(sessionKey: string, venvPath: string): void {
...
function slugifySessionKey(key: string): string {
...
function resolveDefaultRequirements(workspaceDir: string): string | null {
