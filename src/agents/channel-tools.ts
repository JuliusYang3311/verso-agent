...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as VersoConfig);
...
export function listChannelAgentTools(params: { cfg?: VersoConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: VersoConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) return [];
  try {
