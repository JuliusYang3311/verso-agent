...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.VERSO_AGENT_DIR;
    else process.env.VERSO_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-chutes-"));
    process.env.VERSO_STATE_DIR = tempDir;
    process.env.VERSO_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
