...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.VERSO_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: verso --profile isolated doctor --non-interactive",
      );
