...
const PREEMPTIVE_COMPACTION_MARGIN = 1.1;

async function readSessionMessagesFromFile(sessionFile?: string): Promise<AgentMessage[]> {
  if (!sessionFile) {
    return [];
  }
  try {
    ...
    for (const line of raw.split("\n")) {
      ...
      try {
        const entry = JSON.parse(trimmed) as { type?: string; message?: AgentMessage };
        if (entry?.type === "message" && entry.message) {
          messages.push(entry.message);
        }
      } catch {
        // Ignore malformed lines; compaction can still proceed best-effort.
      }
    }
    return messages;
  } catch {
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      await ensureVersoModelsJson(params.config, agentDir);
      ...
      try {
        while (true) {
          ...
          const maxPromptTokens = Math.max(0, ctxInfo.tokens - reserveTokensFloor);
          if (maxPromptTokens > 0) {
            const messages = await readSessionMessagesFromFile(params.sessionFile);
            if (messages.length > 0) {
              const estimatedTokens =
                estimateMessagesTokens(messages) * PREEMPTIVE_COMPACTION_MARGIN;
              if (estimatedTokens > maxPromptTokens) {
                log.warn(
                  `preemptive compaction: session=${redactedSessionId} ` +
                    `tokens=${Math.round(estimatedTokens)} max=${maxPromptTokens} ` +
                    `reserve=${reserveTokensFloor} provider=${provider}/${modelId}`,
                );
                ...
                if (compactResult.compacted) {
                  ...
                  log.info(
                    `preemptive compaction succeeded: ${compactResult.result?.summary ?? "compacted"} ` +
                      `tokens=${compactResult.result?.tokensBefore}→${compactResult.result?.tokensAfter}`,
                  );
                } else {
                  log.warn(
                    `preemptive compaction skipped: ${compactResult.reason ?? "nothing to compact"}`,
                  );
                }
              }
            }
