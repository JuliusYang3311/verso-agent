...
function loadSessionEntryByKeyWithStore(sessionKey: string) {
  const cfg = loadConfig();
  const agentId = resolveAgentIdFromSessionKey(sessionKey);
  const storePath = resolveStorePath(cfg.session?.store, { agentId });
  const store = loadSessionStore(storePath);
  const entry = store[sessionKey];
  return { cfg, entry, storePath };
}
...
async function readLatestAssistantReplyWithRetry(params: {
  ...
  const deadline = Date.now() + Math.max(0, Math.min(params.maxWaitMs, 60_000));
...
export async function runSubagentAnnounceFlow(params: {
  ...
  try {
    ...
    if (!reply?.trim()) {
      ...
      if (params.endedAt || outcome?.status) {
        ...
        const triggerMessage = [
          `A ${announceType} "${taskLabel}" failed to produce any transcript output.`,
          "",
          "Findings:",
          "(no output)",
          "",
          "Reason: child session returned no assistant messages after completion.",
    ...
    const active = childSessionId ? isEmbeddedPiRunActive(childSessionId) : false;
    const childEntry = loadSessionEntryByKey(params.childSessionKey);
    const lastActivity =
      params.endedAt ??
      (typeof childEntry?.updatedAt === "number" ? childEntry.updatedAt : undefined);

    if (lastActivity && Date.now() - lastActivity > 600_000 && !active) {
      if (!finalReply?.trim() && !didAnnounce) {
        const announceType = params.announceType ?? "subagent task";
        const taskLabel = params.label || params.task || "task";
        const triggerMessage = [
          `A ${announceType} "${taskLabel}" failed to produce any transcript output.`,
