...
describe("resolveVersoAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-agent-"));
    process.env.VERSO_STATE_DIR = tempStateDir;
    delete process.env.VERSO_AGENT_DIR;
    delete process.env.PI_CODING_AGENT_DIR;

    const resolved = resolveVersoAgentDir();

    expect(resolved).toBe(path.join(tempStateDir, "agents", "main", "agent"));
  });

  it("honors VERSO_AGENT_DIR overrides", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-agent-"));
    const override = path.join(tempStateDir, "agent");
    process.env.VERSO_AGENT_DIR = override;
    delete process.env.PI_CODING_AGENT_DIR;
