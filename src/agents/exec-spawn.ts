...
import { requestHeartbeatNow } from "../heartbeat/heartbeat-wake.js";
...
export type ExecToolDetails =
  ...
  | {
      ...
      approvalSlug: string;
      expiresAtMs: number;
      host: import("../approval/exec-approvals.js").ExecHost;
      command: string;
      cwd?: string;
      nodeId?: string;
    };
...
export async function runExecProcess(opts: {
  ...
  // Concurrency gate: wait for a spawn slot to prevent EBADF from fd exhaustion
  if (!canSpawnProcess()) {
    logWarn(`exec: concurrency limit reached, waiting for a spawn slot (session=${sessionId})`);
    const gotSlot = await waitForSpawnSlot();
    if (!gotSlot) {
      throw new Error(
        "Too many concurrent processes. Could not acquire a spawn slot within 30 seconds.",
      );
    }
  }
