...
export function isCommandEnabled(cfg: VersoConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: VersoConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: VersoConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: VersoConfig) {
