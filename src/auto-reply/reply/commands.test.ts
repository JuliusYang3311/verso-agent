...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-commands-"));
...
function buildParams(commandBody: string, cfg: VersoConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
