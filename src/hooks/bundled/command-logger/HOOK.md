...
2. **Appends to log file** - Writes a JSON line to `~/.verso/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.verso/logs/commands.log ~/.verso/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/verso`:
   ```
   /home/username/.verso/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
