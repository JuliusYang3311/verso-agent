...
This directory contains hooks that ship with Verso. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.verso/verso.json`:
...
2. Restart gateway: `pkill -9 -f 'verso.*gateway' && pnpm verso gateway`
3. Enable the hook: `verso hooks enable my-hook`
