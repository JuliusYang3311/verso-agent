...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Verso";
  if (/verso/i.test(trimmed)) return trimmed;
  return `${trimmed} (Verso)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.VERSO_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "verso");
  ...
  const binCli = path.join(cwd, "bin", "verso.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.VERSO_TAILNET_DNS?.trim();
