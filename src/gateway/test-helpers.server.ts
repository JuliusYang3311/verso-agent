...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.VERSO_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.VERSO_CONFIG_PATH;
    } else {
      process.env.VERSO_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.VERSO_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.VERSO_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.VERSO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.VERSO_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.VERSO_SKIP_CANVAS_HOST;
    } else {
      process.env.VERSO_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
