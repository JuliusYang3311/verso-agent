...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.VERSO_SKIP_CHANNELS;
    prevSkipGmail = process.env.VERSO_SKIP_GMAIL_WATCHER;
    process.env.VERSO_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.VERSO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.VERSO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.VERSO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
