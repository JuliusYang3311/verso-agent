...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.VERSO_CONFIG_PATH;
  const prevDisableCache = process.env.VERSO_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "verso-canvas-auth-test-"));
  const configPath = path.join(dir, "verso.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.VERSO_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.VERSO_DISABLE_CONFIG_CACHE;
    } else {
      process.env.VERSO_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
