...
export function listSignalAccountIds(cfg: VersoConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: VersoConfig): string {
...
function mergeSignalAccountConfig(cfg: VersoConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: VersoConfig): ResolvedSignalAccount[] {
