...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "VERSO_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "verso-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.VERSO_STATE_DIR = path.join(home, ".verso");
    ...
    await fs.mkdir(path.join(home, ".verso"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.verso${path.sep}media`);
