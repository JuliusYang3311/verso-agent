---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Verso deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Verso cron scheduling for periodic checks, or version status checks on a machine running Verso (laptop, workstation, Pi, VPS).
---

# Verso Host Hardening

## Overview

Assess and harden the host running Verso, then align it to a user-defined risk tolerance without breaking access. Use Verso security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Verso changes the host firewall, SSH, or OS updates; it does not.
...
5. Verso gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Verso can access sensitive data.
...
### 2) Run Verso security audits (read-only)
...
Offer to apply Verso safe defaults (numbered):

1. `openclaw security audit --fix`

Be explicit that `--fix` only tightens Verso defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Verso version/update status (read-only)
...
- Credential hygiene notes (location of Verso creds, prefer disk encryption)
...
After Verso install or first hardening pass, run at least one baseline audit and version check:

- `openclaw security audit`
- `openclaw security audit --deep`
- `openclaw update status`

Ongoing monitoring is recommended. Use the Verso cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
Do not invent CLI flags or imply Verso enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Verso compaction:
