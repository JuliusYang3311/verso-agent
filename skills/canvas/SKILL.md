...
Display HTML content on connected Verso nodes (Mac app, iOS, Android).
...
Place files in the canvas root directory (default `~/verso/canvas/`):

```bash
cat > ~/verso/canvas/my-game.html << 'HTML'
<!DOCTYPE html>
<html>
...
Node is offline. Use `verso nodes list` to find online nodes.
...
The canvas host serves from `/__verso__/canvas/` prefix:

```
http://<host>:18793/__verso__/canvas/index.html  → ~/verso/canvas/index.html
http://<host>:18793/__verso__/canvas/games/snake.html → ~/verso/canvas/games/snake.html
```

The `/__verso__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
