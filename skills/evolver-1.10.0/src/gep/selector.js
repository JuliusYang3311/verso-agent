...
function selectGeneAndCapsule({ genes, capsules, signals, memoryAdvice, driftEnabled }) {
  ...
  // Optional override for diagnostics/controlled experiments; safe by default.
  ...
  const matchedGeneIds = (genes || [])
    .filter((g) => g && g.type === "Gene" && scoreGene(g, signals) > 0)
    .map((g) => g.id);
  const envPreferredAllowed =
    !!envPreferredGeneId &&
    matchedGeneIds.includes(envPreferredGeneId) &&
    (!!driftEnabled || !bannedGeneIds.has(envPreferredGeneId));
  ...
  if (debugSelector) {
    const matchedPreferred =
      !!preferredGeneId &&
      matchedGeneIds.includes(preferredGeneId) &&
    ...
    if (envPreferredGeneId && !envPreferredAllowed) {
      console.error(
        `[SelectorDebug] ignored EVOLVE_PREFERRED_GENE_ID=${envPreferredGeneId} ` +
          `(must be matched candidate${driftEnabled ? "" : " and not banned"})`,
      );
    }
  }
